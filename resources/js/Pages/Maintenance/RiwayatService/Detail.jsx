...
export default function Detail({ record = {}, unit = {}, related = {}, backUrl = "/riwayat-service-unit/service-umum" }) {
    const totalMaintenance = Number(related.totalService || 0) + Number(related.totalBan || 0);
    const deleteRecord = () => {
        if (!window.confirm("Hapus record service umum ini? Data yang sudah dihapus tidak bisa dikembalikan dari halaman ini.")) return;
        router.delete(`/riwayat-service-unit/service-umum/${record.id_key}`);
    };

    return (
        <AdminLayout>
            <Head title={`Detail Service - ${record.nopol || ""}`} />
            ...
            <div className="space-y-5">
                ...
                <section className="rounded-xl bg-slate-950 p-5 text-white shadow-xl shadow-slate-200">
                    <div className="grid gap-5 lg:grid-cols-[1fr_0.75fr_0.75fr_0.75fr]">
                        <div>
                            ...
                            <p className="mt-2 text-sm font-semibold text-slate-300">{unit?.tipe || record.tipe_service || "-"} di area {record.area || "-"}.</p>
                        </div>
                        <div className="rounded-lg border border-white/10 bg-white/10 p-4">
                            ...
                            <p className="mt-2 break-words text-lg font-black">{formatRp(record.total_biaya_service)}</p>
                        </div>
                        <div className="rounded-lg border border-white/10 bg-white/10 p-4">
                            <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-300">
                                <Gauge size={15} />
                            ...
                            <p className="mt-2 text-lg font-black">{record.odo_services || "-"}</p>
                        </div>
                        <div className="rounded-lg border border-white/10 bg-white/10 p-4">
                            <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-300">
                                <PenTool size={15} />
                ...
                <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <InfoItem label="Tanggal Service" value={record.tanggal_services} />
                    <InfoItem label="Mode Service" value={record.mode_service} />
                    <InfoItem label="Tipe Service" value={record.tipe_service} />
                    <InfoItem label="Nama Bengkel" value={record.nama_bengkel} />
                    <InfoItem label="Status Pembayaran" value={record.status_penbayaran} />
                ...
                <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
                    <div className="border-b border-slate-100 px-5 py-4">
                        ...
                        <p className="mt-1 text-xs font-semibold text-slate-500">Catatan inti dari record service yang dipilih.</p>
                    </div>
                    <div className="grid gap-4 p-5 md:grid-cols-2">
                        <InfoItem label="Keluhan" value={record.keluhan} />
                        <InfoItem label="Spare Parts" value={record.spare_parts || record.jenis_spare_parts} />
                        <InfoItem label="Harga Parts" value={formatRp(record.harga_parts)} />
