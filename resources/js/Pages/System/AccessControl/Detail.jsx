        ...
        <div className="space-y-5">
            <section className="rounded-xl bg-slate-950 p-5 text-white shadow-sm sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex min-w-0 items-start gap-3"><Link href={backUrl} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-white/10 bg-white/10 text-white hover:bg-white hover:text-slate-950"><ArrowLeft size={18} /></Link><div><p className="text-xs font-black tracking-wider text-violet-200">Detail User</p><h1 className="mt-2 break-words text-2xl font-black">{userRecord.nik}</h1><p className="mt-1 break-all text-sm font-semibold text-slate-300">{userRecord.email}</p></div></div>
                    <div className="flex flex-wrap gap-2"><Link href={`/system/access-control/users/${userRecord.id}/edit`} className="inline-flex h-10 items-center gap-2 rounded-lg bg-violet-600 px-4 text-sm font-black text-white hover:bg-violet-500"><Edit3 size={16} />Edit User</Link>{!userRecord.isCurrentUser && <button type="button" onClick={destroy} className="inline-flex h-10 items-center gap-2 rounded-lg border border-rose-400/30 bg-rose-500/15 px-4 text-sm font-black text-rose-200 hover:bg-rose-500 hover:text-white"><Trash2 size={16} />Hapus</button>}</div>
                </div>
            </section>
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                <div className="flex items-center gap-2"><User size={18} className="text-violet-600"/><h2 className="text-sm font-black uppercase text-slate-950">Role Utama</h2></div>
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                <div className="flex items-center gap-2"><LockKeyhole size={18} className="text-violet-600"/><h2 className="text-sm font-black uppercase text-slate-950">Akses User</h2></div>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{permissions.map((permission) => { const inherited = isSuperAdmin || rolePermissionNames.has(permission.name); const checked = inherited || directPermissions.includes(permission.name); return <label key={permission.name} className={`flex items-start gap-3 rounded-lg border p-3 ${checked ? "border-violet-200 bg-violet-50/60" : "border-slate-200"}`}><input type="checkbox" checked={checked} disabled={inherited} onChange={() => togglePermission(permission.name)} className="mt-0.5 rounded border-slate-300 text-violet-600 focus:ring-violet-500"/><span><span className="block text-sm font-black text-slate-800">{permission.label}</span><span className="mt-0.5 block text-[11px] font-semibold text-slate-400">{inherited ? "Dari role" : permission.name}</span></span></label>; })}</div>
                <div className="mt-5 flex justify-end"><button type="button" disabled={processing} onClick={saveAccess} className="inline-flex h-10 items-center gap-2 rounded-lg bg-slate-950 px-4 text-sm font-black text-white hover:bg-violet-700 disabled:opacity-50"><Save size={16}/>Simpan Akses User</button></div>
            </section>
        </div>
    </AdminLayout>;
}
