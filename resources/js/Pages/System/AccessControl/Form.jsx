...
export default function Form({ mode = "create", userRecord = {}, roles = [], submitUrl, backUrl }) {
    ...
    const inputClass = "h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-800 outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100";

    return <AdminLayout><Head title={isEdit ? "Edit User" : "Tambah User"}/><div className="mx-auto max-w-3xl space-y-5">
        <section className="rounded-xl bg-slate-950 p-5 text-white shadow-sm"><div className="flex items-start gap-3"><Link href={backUrl} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-white/10 bg-white/10 hover:bg-white hover:text-slate-950"><ArrowLeft size={18}/></Link><div><div className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-wider text-cyan-200"><UserPlus size={15}/>{isEdit ? "Edit User" : "User Baru"}</div><h1 className="mt-2 text-2xl font-black">{isEdit ? userRecord.nik : "Tambah Pengguna"}</h1></div></div></section>
        ...
        </div><div className="mt-6 flex justify-end"><button type="submit" disabled={processing} className="inline-flex h-11 items-center gap-2 rounded-lg bg-cyan-600 px-5 text-sm font-black text-white hover:bg-cyan-700 disabled:opacity-50"><Save size={16}/>{isEdit ? "Simpan Perubahan" : "Buat Pengguna"}</button></div></form>
    </div></AdminLayout>;
}
