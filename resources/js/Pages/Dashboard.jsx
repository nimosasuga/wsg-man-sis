...
export default function Dashboard({ dbChartData }) {
    ...
    const activeShipmentYear = shipmentYears.includes(Number(selectedYear)) ? Number(selectedYear) : shipmentYears[0];
    const shipmentTotal = shipmentData.find((item) => Number(item.tahun) === Number(activeShipmentYear))?.months?.reduce((sum, item) => sum + Number(item.value || 0), 0) || 0;
